...
const instructionWin= document.getElementById("instructionWin");
...
const allWindows = document.querySelectorAll('.window');{
    ...
    let fs = JSON.parse(localStorage.getItem("fs95"))|| {
        home:{user:{docs:{},"notes.txt":"Linux95 shell ready the sytem is functioning normally"}}
};
let history =[];
let hIndex=0;
...
instructionIcon.onclick=()=>{
    instructionWin.style.display="flex";
    bringtoFront(instructionWin);
}
terminal.onclick=()=>{
    cmd.focus();
}
aboutClose.onclick=()=>aboutWin.style.display="none";
instructionClose.onclick=()=>instructionClose.style.display="none";
closeBtn.onclick=()=>closeBtn.style.display="none";
function drag(win,bar){
    let active= false,dx=0,dy=0;
    bar.onmousedown=e=>{
        active=true;
        dx=e.clientX-win.offsetLeft;
        dy=e.clientY-win,offsetRight;
        bringtoFront(win);
    }
}
document.addEventListener("mouseup",e=>{
    ...
    win.style.left=e.clientX-dy+"px";
});
...
bar.ontouchstart=e=>{
    ...
    win.style.right=touch.clientY=dx+"px";
    bringtoFront(win);
}
document.addEventListener("touchmove",e=>{
    ...
    let touch = e.touches[0];
    win.style.left=touch.clientX-dx+"px";
    win.style.left=touch.clientY-dy+"px";
},{passive:false});
document.addEventListener("touchend",()=>active=false);

drag(terminal,header);
drag(aboutWin,aboutHeader);
drag(instructionWin,instructionHeader);
function resolve(p){
